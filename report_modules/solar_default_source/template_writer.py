...
class TemplateWriter(object):
    ...
    def __init__(self,
        ...
        self.total_page_count += 6 # 6 pages for common and summary pages + 2 pages for chart pages.
    ...
    def generate_front_page(self):
        ...
        project_details = self.payload.get("ProjectDetails", {})
        
        html = front_page_template.render({
                                    "base_dir":self.templates_path,
                                    "site_name":project_details.get("SiteName"),
                                    "project_name":project_details.get("ProjectName").upper(),
                                    "plant_capacity": str(self.payload.get("PlantCapacity", 0) / 1_000_000) + ' MW',
                                    "survey_name": project_details.get("SurveyName")
                                    })
    ...
    def generate_summary_page(self):
        ...
        anomaly_counts = self.payload.get("TotalAnomaliesCounts", {})
        ...
        asset_labels = [name for name, _ in self.payload.get("AssetWiseAnomaliesCount", {}).items()]
        asset_data   = [count for _, count in self.payload.get("AssetWiseAnomaliesCount", {}).items()] 
        ...
        html = summary_page_template.render({
            ...
            "annomaly_details": [
                                    {"AnnomalyName": k, "AnnomalyCount": v}
    ...
    def generate_annotation_page(self):
        def extract_line_box_coordinates(line_coords: list, img_width: int, padding: int = 30):
            """
            Extract a padded bounding box around coordinates that are already in image space.
            """
        ...
        def extract_line_coordinates(ui_coord: list, img_height: int) -> List[Tuple[float,float]]:
            return [(float(x), float(img_height - y)) for x, y in ui_coord]
        ...
        images = []
        # for asset in self.payload.get("AssetDetails"):
        for section_name, section_data in self.payload.get("AssetDetails", {}).items():
            for anno in section_data:
                if anno.get("GeometryVisual") or anno.get("GeometryThermal"):
                    anno["Direction"] = section_name
                    images.append(anno)
        ...
        for image_count, annotation in enumerate(images):
            ...
            image_id = f"image_{image_count + 1}"
            current_image_dir = os.path.join(self.project_dir, str(image_id))
            os.makedirs(current_image_dir, exist_ok=True)
            
            
            image_sources = []
            if annotation.get("GeometryVisual") and annotation.get("ImageVisualUrl"):
                image_sources.append(("visual", annotation.get("ImageVisualUrl")))
            if annotation.get("GeometryThermal") and annotation.get("ImageThermalUrl"):
                image_sources.append(("thermal", annotation.get("ImageThermalUrl")))
                
            if not image_sources:
                print("No Visual Or thermal image found")
                continue
            ...
            for image_type, image_url in image_sources:
                ...
                if image_type == "thermal":
                    annotation_payload = annotation.get("GeometryThermal")
                else:
                    annotation_payload = annotation.get("GeometryVisual")

                if not annotation_payload:
                    print(f"    No geometry found for {image_type}")
                    continue
                
                # imagePayload may already be a dict
                if isinstance(annotation_payload.get("imagePayload"), str):
                    try:
                        annotation_payload["imagePayload"] = json.loads(annotation_payload["imagePayload"])
                    except Exception:
                        pass
                ...
                if geometry_type == "LineString":
                    line_coord = extract_line_coordinates(geometry_coords, img_height)
                    box_coord = extract_line_box_coordinates(line_coord, img_width=img_width, padding=10)
                    ...
                    if image_type == "thermal":
                        annotation["thermal_image_path"] = annot_img_save_path
                    else:
                        annotation["visual_image_path"] = annot_img_save_path
                        
                    file_name = os.path.basename(image_url or "image.jpg")
                    base_name = os.path.splitext(file_name)[0]

                    img_save_path = os.path.join(
                        current_image_dir,
                        f"{image_type}_{base_name}_{anomaly_count}.jpg"
                    )
                    img = img.convert("RGB")
                    img.save(img_save_path)
                    
                    if image_type == "thermal":
                        annotation["thermal_parent_img_path"] = img_save_path
                    else:
                        annotation["visual_parent_img_path"] = img_save_path
                    ...
                    if image_type == "thermal":
                        annotation["thermal_image_path"] = annot_img_save_path
                        file_name = os.path.basename(image_url or "thermal_image.jpg")
                    else:
                        annotation["visual_image_path"] = annot_img_save_path
                        file_name = os.path.basename(image_url or "visual_image.jpg")
                ...
                print(f"{Fore.RED} [template_writer] > generate_anomaly_page {Fore.RESET} |  image could not be downloaded :")
        ...
        for image_count, annotation in enumerate(images):
            print(f"Generating PDF of ... {Fore.YELLOW}{ image_count + 1 }{Fore.RESET}/{Fore.BLUE}{total_annotated_images}{Fore.RESET} ")
            html = annotation_page_template.render({
                                    "base_dir":self.templates_path,
                                    # "InspectionName": self.payload.get("InspectionName"),
                                    # "inspection_date": self.payload.get("DateRange"),
                                    # "tower_data": tower,
                                    "annotation":annotation,
                                    "tower_no": "&nbsp",
                                    
                                    # "tower_lat": tower["ObjectStructureGeoLocation"]["coordinates"][0],
                                    # "tower_long": tower["ObjectStructureGeoLocation"]["coordinates"][1],
                                    "base_url": "https://coredatarepo.s3.ap-south-1.amazonaws.com/",
                                    "page_index": self.current_page_count,
                                    "total_pages": self.total_page_count,
                                    # "position_of_defect": image.get("AssetPath", "N/A"),
                                    "footer_inspection_name":self.payload.get("SurveyName"),
                                    "footer_current_date": self.current_date
                                    })
            ...
            annotation_id = annotation.get("annotation_id", image_count+1)
            pdf_path = os.path.join(self.project_dir, f"{inspection_name}_{annotation_id}.pdf")
