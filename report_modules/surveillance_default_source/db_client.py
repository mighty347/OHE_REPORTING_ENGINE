...
class DbClient():
    ...
    def get_solar_report_data(self, survey_id:str,objectstructure_id:str,annomalies:str)  :
        result = None
        try:
            ...
            cursor.callproc('getsolarreportbyobjectstructureids', [survey_id,objectstructure_id,annomalies,])
    ...
    def get_surveillance_report_data(self,p_from: str,p_to: str, p_site_ids: list, p_zone_ids: list, p_camera_ids: list,p_anomaly_ids: list):
        result = None
        try:
            print(f"connecting to pg_admin via connection string : {self.conn_args}")
            connection = psycopg2.connect(**self.conn_args)
            ...
            cursor = connection.cursor()
            print("calling stored procedure.")
            # print(f"from_date : {from_date}, to_date : {to_date}, site_id : {site_id}, zone_id : {zone_id}, camera_id : {camera_id}, anomaly_id : {anomaly_id}")
            print(f"{p_from}, {p_to}, {p_site_ids}, {p_zone_ids}, {p_camera_ids}, {p_anomaly_ids}")
            # cursor.callproc('sp_get_annotations_for_report', [p_from, p_to, p_site_ids, p_zone_ids, p_camera_ids, p_anomaly_ids])
            query = """
        SELECT *
        FROM sp_get_annotations_for_report(
            %s::timestamptz,
            %s::timestamptz,
            %s::uuid[],
            %s::uuid[],
        ...
        """
            ...
            cursor.execute(
                query,
                (p_from, p_to, p_site_ids, p_zone_ids, p_camera_ids, p_anomaly_ids)
            )
            ...
            try:
                result = data # get the first element of the tuple
                result = {"surveillance_reports": result}
